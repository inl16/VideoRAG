...
export const runQuery = async (query) => {
  // Change runQuery to accept a query parameter
  try {
...
<important>
IMPORTANT: You MUST cite your sources (video name, timestamp range, video url) - only use one video no more than twice - make sure to use others if you need to 
</important>`,
      },
    ];
    ...
    const result = await anthropic.messages.create({
      model: "claude-3-5-sonnet-20240620",
      system: `<task>
Given relevant videos as context, use the context to answer the user's query and site the sources of your answer in the format. The answer should be a long, well-thought-out response:
...
<important>
IMPORTANT: You MUST cite your sources (video name, timestamp range, video url)
Note: If the video content you are referencing only has one segment or you are referencing the last segment in a video, the timestamp range should end with "end of video" only give answers in Spanish. 
    ...
    return answer; // Return the answer to be sent back to Voiceflow
  } catch (error) {
